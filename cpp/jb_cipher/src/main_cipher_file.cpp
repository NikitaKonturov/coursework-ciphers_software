#include "cipher_first\jb-cipher.hpp"
...
PYBIND11_MODULE(jb_cipher, m) {
    m.doc() = "Шифр Джефферсона-Базери"; // документация к модулю питона, обязана содержать имя библиотеки на английском которое будет отображаться во frontend
