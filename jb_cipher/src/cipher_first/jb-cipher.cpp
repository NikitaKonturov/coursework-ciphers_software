...
std::vector<int32_t> get_cyclic_substitution(const std::vector<int32_t>& trivialPermut, uint32_t shift) {
...
std::map<std::wstring, std::wstring> encript(std::vector<std::wstring> openTexts, std::vector<std::wstring> keys)
{
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        if(openTexts[i].size() % key.size() != 0) {
            throw InvalidOpenText("Open text size must be a multiple of disk count...");
        }
        ...
        Permutation cyclic_permut(get_cyclic_substitution(trivialPermut, barShift)); 
        ...
        wss << keys[i] << L"\n";
        for (size_t j = 0; i < trivialDisksPermutation.size(); ++j) {
...
std::map<std::wstring, std::wstring> decript(std::map<std::wstring, std::wstring> keysAndCipherTexts)
{
    ...
    for(auto& keyAndCipherText : keysAndCipherTexts) {
        ...
        std::getline(wss, key);
        std::wregex permutReg(L"\\[\\d+(?:\\s+\\d+)*\\]");

        std::wsregex_iterator iter(key.begin(), key.end(), permutReg);
        std::wsregex_iterator end;

        std::vector<Permutation> trivialPermutationOfpermutationsOnDisks;
        while (iter != end) {
            trivialPermutationOfpermutationsOnDisks.push_back(Permutation(iter->str()));
        }
        
        Permutation keyPermutation = trivialPermutationOfpermutationsOnDisks[trivialPermutationOfpermutationsOnDisks.size() - 1];
        trivialPermutationOfpermutationsOnDisks.pop_back();
        ...
        for (size_t j = 0; j < permutationOfsubstitutionsOnDisks.size(); ++j) {
            permutationOfsubstitutionsOnDisks[j] = get_alfabet_substitution(permutationOfpermutationsOnDisks[j], Permutation(get_cyclic_substitution(trivialPermut, std::abs(static_cast<int32_t>(trivialPermut.size()) - barShift))));
        }
...
void chekRequest(nlohmann::json keyPropertys)
{
    try {
        ...
        if(!keyPropertys.at("disk_count").is_number()) {
            throw KeyPropertyError("Key disk_count must has number value...");
        }
        if(keyPropertys.at("disk_count") <= 0) {
            throw InvalidKey("Value disk_count must be more 0...");
        }   
